...
export default function TimetableGrid({ 
  ...
  const getBlocksForSlot = (hour: number, minute: number) => {
    ...
    return timetable.filter(block => {
      ...
      if (block.type === 'task' && block.task && block.task.completed) {
        return false;
      }
    ...
    switch (block.type) {
      case 'task':
        switch (block.task?.type) {
          case 'work': return <Briefcase className="w-3 h-3 flex-shrink-0" />;
          case 'study': return <BookOpen className="w-3 h-3 flex-shrink-0" />;
          case 'leisure': return <Gamepad2 className="w-3 h-3 flex-shrink-0" />;
          default: return <Clock className="w-3 h-3 flex-shrink-0" />;
        }
      case 'break': return <Coffee className="w-3 h-3 flex-shrink-0" />;
      case 'unavailable': return <Ban className="w-3 h-3 flex-shrink-0" />;
      default: return <Clock className="w-3 h-3 flex-shrink-0" />;
    }
  };
  ...
  const handleDrop = (e: React.DragEvent, hour: number, minute: number) => {
    ...
    if (hour < 6 || hour >= 23) {
      toast({
        ...
        description: "Tasks can only be scheduled between 6:00 AM and 11:00 PM",
        variant: "destructive"
      });
      setDraggedTask(null);
      return;
  ...
  const handleSlotClick = (hour: number, minute: number) => {
    ...
    // Check if time is within working hours (6 AM to 11 PM)
    if (hour < 6 || hour >= 23) {
      toast({
        title: "Outside working hours",
        description: "Time blocks can only be scheduled between 6:00 AM and 11:00 PM",
        variant: "destructive"
      });
      return;
    }
  ...
  useEffect(() => {
    ...
    // Scroll the grid so the current slot is at the top (with offset to prevent overshooting)
  ...
  const handleBookTime = () => {
    ...
    // Ensure the booking doesn't extend past working hours (11 PM)
    const dayEnd = new Date(selectedDate);
    dayEnd.setHours(23, 0, 0, 0);
    
    if (endTime > dayEnd) {
      toast({
        title: "Booking extends past working hours",
        description: "Please reduce the duration or choose an earlier time. Working hours end at 11:00 PM.",
        variant: "destructive"
      });
      return;
    }
          ...
          return (
            <div
              ...
              className={cn(
                ...
                draggedTask && isEmpty && !isPast && "bg-green-500/10 border-green-500/20",
                draggedTask && !isEmpty && "bg-red-500/10 border-red-500/20",
                isPast && "pointer-events-none"
              )}
              onDragOver={!isPast && isEmpty ? handleDragOver : undefined}
              onDrop={!isPast && isEmpty ? (e) => handleDrop(e, hour, minute) : undefined}
              onClick={!isPast ? () => handleSlotClick(hour, minute) : undefined}
            >
              {/* Time column */}
              <div className={cn(
                "w-16 md:w-20 p-2 bg-muted/20 border-r border-border text-xs md:text-sm font-mono flex items-center shrink-0",
                        ...
                        <div className="flex items-start gap-1 w-full">
                          ...
                          {(block.type === 'unavailable' || (block.type === 'task' && !block.isFixed)) && (
                            <Button
                              size="sm"
                              variant="ghost"
                              className="opacity-0 group-hover:opacity-100 h-5 w-5 p-0 hover:bg-destructive/20 ml-1 flex-shrink-0"
                              onClick={(e) => {
                                e.stopPropagation();
